...
class DCN(object):
    ...
    def _coordinate_map_3D(self, offset, if_offset):
        ...
        # offset
        offset1, offset2 = torch.split(offset, 3*self.num_points, dim=1) # Split offset into groups of self.num_points i.e. [N, 3*K, D, W, H]
        ...
        if self.morph == 0:
            ...
            if if_offset: # x_new are positions, offsetX are vectors in (-1, 1)
                ...
                z_new[center + 1] = z_new[center] + offset1[(center + 1) * 3 + 1]
                y_new[center + 1] = y_new[center] + offset2[(center + 1) * 3 + 1]
                z_new[center - 1] = z_new[center] + offset1[(center - 1) * 3 + 1]
                y_new[center - 1] = y_new[center] + offset2[(center - 1) * 3 + 1] # for center to center
                z_new = z_new.clamp(min=0, max=self.depth)
                y_new = y_new.clamp(min=0, max=self.width)

                for index in range(2, center + 1):
                    #offset_cat_pos = torch.stack([offset1[(center + index) * 3], offset1[(center + index) * 3 + 1], offset1[(center + index) * 3 + 2], offset2[(center + index) * 3], offset2[(center + index) * 3 + 1], offset2[(center + index) * 3 + 2]], dim=0)
                    offset_cat_pos = torch.cat((offset1[(center + index) * 3 : (center + index) * 3 + 3], offset2[(center + index) * 3 : (center + index) * 3 + 3]), dim=0) # [6, N, D, W, H]
                    ...
                    #z_offset_pos = self._offset_interpolate_3D(offset1[(center + index) * 3 : (center + index) * 3 + 3], z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                    #y_offset_pos = self._offset_interpolate_3D(offset2[(center + index) * 3 : (center + index) * 3 + 3], z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                    z_index_pos = torch.round(z_new[center + index - 1] - z_new[center + index - 2]).long() + 1 # [N, D, W, H]
                    y_index_pos = torch.round(y_new[center + index - 1] - y_new[center + index - 2]).long() + 1 # [N, D, W, H]
                    #z_new[center + index] = (z_new[center + index - 1] + z_offset_pos[z_index_pos]).clamp(min=0, max=self.depth)
                    #y_new[center + index] = (y_new[center + index - 1] + y_offset_pos[y_index_pos]).clamp(min=0, max=self.width)
                    z_new[center + index] = (z_new[center + index - 1] + torch.gather(z_offset_pos, 0, z_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.depth) # [N, D, W, H] for each element of z_new
                    y_new[center + index] = (y_new[center + index - 1] + torch.gather(y_offset_pos, 0, y_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.width) # [N, D, W, H] for each element of y_new
                    
                    #offset_cat_neg = torch.stack([offset1[(center - index) * 3], offset1[(center - index) * 3 + 1], offset1[(center - index) * 3 + 2], offset2[(center - index) * 3], offset2[(center - index) * 3 + 1], offset2[(center - index) * 3 + 2]], dim=0)
                    offset_cat_neg = torch.cat((offset1[(center - index) * 3 : (center - index) * 3 + 3], offset2[(center - index) * 3 : (center - index) * 3 + 3]), dim=0) # [6, N, D, W, H]
                    ...
                    #z_offset_neg = self._offset_interpolate_3D(offset1[(center - index) * 3 : (center - index) * 3 + 3], z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
                    #y_offset_neg = self._offset_interpolate_3D(offset2[(center - index) * 3 : (center - index) * 3 + 3], z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
                    z_index_neg = torch.round(z_new[center - index + 1] - z_new[center - index + 2]).long() + 1 # [N, D, W, H]
                    y_index_neg = torch.round(y_new[center - index + 1] - y_new[center - index + 2]).long() + 1 # [N, D, W, H]
                    #z_new[center - index] = (z_new[center - index + 1] + z_offset_neg[z_index_neg]).clamp(min=0, max=self.depth)
                    #y_new[center - index] = (y_new[center - index + 1] + y_offset_neg[y_index_neg]).clamp(min=0, max=self.width)
                    z_new[center - index] = (z_new[center - index + 1] + torch.gather(z_offset_neg, 0, z_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.depth) # [N, D, W, H] for each element of z_new
                    y_new[center - index] = (y_new[center - index + 1] + torch.gather(y_offset_neg, 0, y_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.width) # [N, D, W, H] for each element of y_new
            ...
            if if_offset:
                ...
                x_new[center + 1] = x_new[center] + offset1[(center + 1) * 3 + 1]
                z_new[center + 1] = z_new[center] + offset2[(center + 1) * 3 + 1]
                x_new[center - 1] = x_new[center] + offset1[(center - 1) * 3 + 1]
                z_new[center - 1] = z_new[center] + offset2[(center - 1) * 3 + 1]
                ...
                for index in range(2, center + 1):
                    #offset_cat_pos = torch.stack([offset1[(center + index) * 3], offset1[(center + index) * 3 + 1], offset1[(center + index) * 3 + 2], offset2[(center + index) * 3], offset2[(center + index) * 3 + 1], offset2[(center + index) * 3 + 2]], dim=0)
                    offset_cat_pos = torch.cat((offset1[(center + index) * 3 : (center + index) * 3 + 3], offset2[(center + index) * 3 : (center + index) * 3 + 3]), dim=0)
                    ...
                    #x_offset_pos = self._offset_interpolate_3D(offset1[(center + index) * 3 : (center + index) * 3 + 3], z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                    #z_offset_pos = self._offset_interpolate_3D(offset2[(center + index) * 3 : (center + index) * 3 + 3], z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                    x_index_pos = torch.round(x_new[center + index - 1] - x_new[center + index - 2]).long() + 1
                    z_index_pos = torch.round(z_new[center + index - 1] - z_new[center + index - 2]).long() + 1
                    #x_new[center + index] = (x_new[center + index - 1] + x_offset_pos[x_index_pos]).clamp(min=0, max=self.height)
                    #z_new[center + index] = (z_new[center + index - 1] + z_offset_pos[z_index_pos]).clamp(min=0, max=self.depth)
                    x_new[center + index] = (x_new[center + index - 1] + torch.gather(x_offset_pos, 0, x_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.height)
                    z_new[center + index] = (z_new[center + index - 1] + torch.gather(z_offset_pos, 0, z_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.depth)
                    
                    #offset_cat_neg = torch.stack([offset1[(center - index) * 3], offset1[(center - index) * 3 + 1], offset1[(center - index) * 3 + 2], offset2[(center - index) * 3], offset2[(center - index) * 3 + 1], offset2[(center - index) * 3 + 2]], dim=0)
                    offset_cat_neg = torch.cat((offset1[(center - index) * 3 : (center - index) * 3 + 3], offset2[(center - index) * 3 : (center - index) * 3 + 3]), dim=0)
                    ...
                    #x_offset_neg = self._offset_interpolate_3D(offset1[(center - index) * 3 : (center - index) * 3 + 3], z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
                    #z_offset_neg = self._offset_interpolate_3D(offset2[(center - index) * 3 : (center - index) * 3 + 3], z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
                    x_index_neg = torch.round(x_new[center - index + 1] - x_new[center - index + 2]).long() + 1
                    z_index_neg = torch.round(z_new[center - index + 1] - z_new[center - index + 2]).long() + 1
                    #x_new[center - index] = (x_new[center - index + 1] + x_offset_neg[x_index_neg]).clamp(min=0, max=self.height)
                    #z_new[center - index] = (z_new[center - index + 1] + z_offset_neg[z_index_neg]).clamp(min=0, max=self.depth)
                    x_new[center - index] = (x_new[center - index + 1] + torch.gather(x_offset_neg, 0, x_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.height)
                    z_new[center - index] = (z_new[center - index + 1] + torch.gather(z_offset_neg, 0, z_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.depth)
            ...
            if if_offset:
                ...
                for index in range(2, center + 1):
                    ...
                    #y_offset_pos = self._offset_interpolate_3D(offset2[(center + index) * 3 : (center + index) * 3 + 3], z_new[center + index - 1], y_new[center + index - 1], x_new[center + index - 1])
                    x_index_pos = torch.round(x_new[center + index - 1] - x_new[center + index - 2]).long() + 1
                    y_index_pos = torch.round(y_new[center + index - 1] - y_new[center + index - 2]).long() + 1
                    #x_new[center + index] = (x_new[center + index - 1] + x_offset_pos[x_index_pos]).clamp(min=0, max=self.height)
                    #y_new[center + index] = (y_new[center + index - 1] + y_offset_pos[y_index_pos]).clamp(min=0, max=self.width)
                    x_new[center + index] = (x_new[center + index - 1] + torch.gather(x_offset_pos, 0, x_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.height)
                    y_new[center + index] = (y_new[center + index - 1] + torch.gather(y_offset_pos, 0, y_index_pos.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.width)
                    ...
                    #y_offset_neg = self._offset_interpolate_3D(offset2[(center - index) * 3 : (center - index) * 3 + 3], z_new[center - index + 1], y_new[center - index + 1], x_new[center - index + 1])
                    x_index_neg = torch.round(x_new[center - index + 1] - x_new[center - index + 2]).long() + 1
                    y_index_neg = torch.round(y_new[center - index + 1] - y_new[center - index + 2]).long() + 1
                    #x_new[center - index] = (x_new[center - index + 1] + x_offset_neg[x_index_neg]).clamp(min=0, max=self.height)
                    #y_new[center - index] = (y_new[center - index + 1] + y_offset_neg[y_index_neg]).clamp(min=0, max=self.width)
                    x_new[center - index] = (x_new[center - index + 1] + torch.gather(x_offset_neg, 0, x_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.height)
                    y_new[center - index] = (y_new[center - index + 1] + torch.gather(y_offset_neg, 0, y_index_neg.unsqueeze(0)).squeeze(0)).clamp(min=0, max=self.width)
