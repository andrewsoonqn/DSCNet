...
class DCN_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph, if_offset, device):
        ...
        self.offset_conv9r = nn.Conv3d(in_ch, 3, 9, padding=4)
    ...
    def forward(self, f):
        ...
        offset9 = self.offset_conv9(f)
        offset9r = self.offset_conv9r(f)
        offset = torch.concat([offset9r, offset7r, offset5r, offset3r, offset0, offset3, offset5, offset7, offset9], dim=1) # Output: [N, 3*C, D, W, H]
