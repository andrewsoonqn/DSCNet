...
from dscnet.workflow import Process
...
def _control_evidence(resolved) -> tuple[dict, dict, str, str]:
    ...
    return dataset, provenance, environment["uv_lock_sha256"], digest


def _lock_identifier() -> str:
    lock = REPO_ROOT / "uv.lock"
    if not lock.is_file():
        raise RuntimeError("uv.lock is required for reproducible experiments")
...
def _execute(config, resolved, parent_run_id: str | None = None) -> dict[str, str]:
    ...
    if os.environ.get("DSCNET_CONTROL_DIR"):
        ...
        lock_identifier = _lock_identifier()
