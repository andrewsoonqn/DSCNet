...
After shadow-mode equivalence, remove duplicate configuration ownership from `dscnet/workflow.py` and `scripts/run_slurm.sh`. Test interruption, resume, awkward-volume inference, and retrieval before treating the workflow as experiment-ready.
...
- add dscnet/experiment/config.py -- typed schemas and semantic validation
- add dscnet/experiment/tracking.py -- MLflow and provenance boundary
- add tools/expctl.py -- deterministic verify, submit, inspect, and fetch CLI
- add .pi/extensions/experiment-control/ -- narrow Pi wrapper around expctl
- add tests/experiment/test_config.py -- schema and ignored-option failures
- add tests/experiment/test_tracking.py -- recorded provenance contract
- add tests/tools/test_expctl.py -- fake SSH and Slurm lifecycle
- modify dscnet/workflow.py -- temporary adapter, then Hydra entry point
...
- modify the parent UROP AGENTS.md -- human workflow and recovery rules
</FileTree>
