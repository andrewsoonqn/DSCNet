...
class DCN_Conv(nn.Module):
    def __init__(self, in_ch, out_ch, kernel_size, extend_scope, morph, if_offset, device):
        ...
        # hardcoded for kernel_size = 9
        self.kernel_size = 4
        self.offset_conv1 = nn.Conv3d(in_ch, 3, 3, padding=1)
        #self.offset_conv1r = nn.Conv3d(in_ch, 3, 3, padding=1)
        self.offset_conv2 = nn.Conv3d(in_ch, 3, 5, padding=2)
        #self.offset_conv2r = nn.Conv3d(in_ch, 3, 5, padding=2)
        self.offset_conv3 = nn.Conv3d(in_ch, 3, 7, padding=3)
        #self.offset_conv3r = nn.Conv3d(in_ch, 3, 7, padding=3)
        self.offset_conv4 = nn.Conv3d(in_ch, 3, 9, padding=4)
        #self.offset_conv4r = nn.Conv3d(in_ch, 3, 9, padding=4)
        #self.bn = nn.BatchNorm3d(3 * kernel_size) # Normalizes across the Channel dimension; returns same shape as input
    ...
    def forward(self, f):
        ...
        offset1 = self.offset_conv1(f)
        #offset1r = self.offset_conv1r(f)
        offset2 = self.offset_conv2(f)
        #offset2r = self.offset_conv2r(f)
        offset3 = self.offset_conv3(f)
        #offset3r = self.offset_conv3r(f)
        offset4 = self.offset_conv4(f)
        #offset4r = self.offset_conv4r(f) # Output: [N, 3, D, W, H]; 
        offset = torch.concat([offset1, offset2, offset3, offset4], dim=1) # Output: [N, 3*C, D, W, H]
        print(offset.shape)
